    ...
    <x-slot:rows>
        @foreach ($clients as $result)
            <tr>
                ...
                {{-- {{$rows}} --}}
                <td>
                    <a @click.prevent.stop="$dispatch('linkaction', {link: '{{route('clients.show', $result->id)}}'})"
                        class="link no-underline hover:underline" href="">
                        {{ $result->client_code }}
                    </a>
                </td>
                <td>
                    <a @click.prevent.stop="$dispatch('linkaction', {link: '{{route('clients.show', $result->id)}}'})"
                        class="link no-underline hover:underline" href="{{route('clients.show', $result->id)}}">
                        {{ $result->name }}
                    </a>
                </td>
